...
$(document).ready(function () {
    ...
    // Настройка AJAX для всех запросов
    $.ajaxSetup({
        headers: {
    ...
    // Обработка всех форм, отправляющих данные в Amo
    $(document).on('submit', 'form[action^="amo/"]', function(e) {
        e.preventDefault();
        
        var form = $(this);
        var formData = form.serialize();
        var loadingBlock = form.closest('.popup, .stock_form').find('.loader_block');
        ...
        // Отправляем данные
            ...
            success: function(response) {
                ...
                if (response.success) {
                    // Скрываем текущую форму и показываем форму успеха
                    if (form.closest('.popup').length) {
                        form.closest('.popup_wrapper').removeClass('active');
                    }
    ...
    $(document).on('submit', 'form.promocode', function(e) {
        e.preventDefault();
        
        var form = $(this);
        var formData = form.serialize();
        var promocodeError = $('#promocode_error');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    // Показываем скидку
                    var priceElement = form.closest('.my_order, .order_program').find('.h5.price, .order_info_price b');
                    
                    if (priceElement.length) {
                        // Форматируем вывод цены
                        priceElement.html('<strike>' + response.original_price + '₽</strike> ' + response.discounted_price + '₽');
                    }
                    
                    // Добавляем информацию о скидке
                    form.find('input[name="promocode"]').prop('disabled', true);
            ...
            error: function() {
                promocodeError.text('Ошибка сервера при проверке промокода').css('color', 'red');
            }
        });
    });
...
$(".get_new_order_btn").click(function () {
    ...
    let order_data = {
        data: {
            ...
            img: $(".ration_item.active .ration_img img").attr("src") || "/files/slim_img.png",
    ...
    console.log('Данные заказа:', order_data);
